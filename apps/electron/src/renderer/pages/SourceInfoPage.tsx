...
function formatRelativeTime(
  ...
  if (!timestamp) return t('sourceInfo.never')
  ...
  if (minutes < 1) return t('sourceInfo.justNow')
  if (minutes < 60) return t(minutes === 1 ? 'sourceInfo.minutesAgo' : 'sourceInfo.minutesAgoPlural', { count: minutes })
  if (hours < 24) return t(hours === 1 ? 'sourceInfo.hoursAgo' : 'sourceInfo.hoursAgoPlural', { count: hours })
  return t(days === 1 ? 'sourceInfo.daysAgo' : 'sourceInfo.daysAgoPlural', { count: days })
}
...
function getConnectionDescription(
  ...
  if (type === 'mcp') {
    if (mcp?.transport === 'stdio') {
      return t('sourceInfo.connectionDescriptionMcpStdio')
    }
    return t('sourceInfo.connectionDescriptionMcp')
  }
  if (type === 'api') {
    return t('sourceInfo.connectionDescriptionApi')
  }
  if (type === 'local') {
    return t('sourceInfo.connectionDescriptionLocal')
  }
  return t('sourceInfo.connectionDescriptionDefault')
}
...
function getPermissionsDescription(
  ...
  if (type === 'mcp') {
    return t('sourceInfo.permissionsDescriptionMcp')
  }
  if (type === 'api') {
    return t('sourceInfo.permissionsDescriptionApi')
  }
  return t('sourceInfo.permissionsDescriptionDefault')
}
...
export default function SourceInfoPage({ sourceSlug, workspaceId, onDelete }: SourceInfoPageProps) {
  ...
  useEffect(() => {
    ...
    const loadSource = async () => {
      try {
        ...
        if (!isMounted) return
        setError(err instanceof Error ? err.message : t('sourceInfo.failedLoad'))
      } finally {
    ...
    const loadTools = async () => {
      try {
        ...
        if (result.success && result.tools) {
          ...
          setMcpToolsError(result.error || t('sourceInfo.failedLoadTools'))
        }
      } catch (err) {
        if (!isMounted) return
        setMcpToolsError(err instanceof Error ? err.message : t('sourceInfo.failedLoadTools'))
      } finally {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('sourceInfo.failedDelete'), {
        ...
        <Info_Page.Content>
          ...
          {source.config.mcp?.transport === 'stdio' && !localMcpEnabled && (
            <Info_Alert variant="warning" icon={<AlertCircle className="h-4 w-4" />}>
              <Info_Alert.Title>{t('sourceInfo.sourceDisabledTitle')}</Info_Alert.Title>
          ...
          {/* Connection */}
          <Info_Section
            title={t('sourceInfo.connectionTitle')}
            ...
            actions={
              ...
              <EditPopover
                trigger={<EditButton />}
                {...getEditConfig('source-config', source.folderPath, t)}
                secondaryAction={{
          ...
          >
            ...
            >
              <Info_Table.Row label={t('sourceInfo.labels.type')} value={source.config.type.toUpperCase()} />
              {sourceUrl && (
                <Info_Table.Row label={t('sourceInfo.labels.url')}>
                  <button
              ...
              <Info_Table.Row label={t('sourceInfo.labels.lastTested')} value={formatRelativeTime(source.config.lastTestedAt, t)} />
            </Info_Table>
          </Info_Section>
          ...
          {source.config.type !== 'mcp' && permissionsConfig && apiPermissionsData.length > 0 && (
            ...
            >
              <PermissionsDataTable data={apiPermissionsData} fullscreen fullscreenTitle={t('sourceInfo.permissionsTitle')} />
            </Info_Section>
          )}
          ...
          {source.config.type === 'mcp' && permissionsConfig && mcpPermissionsData.length > 0 && (
            ...
            >
              <PermissionsDataTable data={mcpPermissionsData} hideTypeColumn fullscreen fullscreenTitle={t('sourceInfo.permissionsTitle')} />
            </Info_Section>
          )}
          ...
          {source.guide?.raw && (
            <Info_Section
              title={t('sourceInfo.documentationTitle')}
              ...
              actions={
                ...
                <EditPopover
                  trigger={<EditButton />}
                  {...getEditConfig('source-guide', source.folderPath, t)}
                  secondaryAction={{
                    label: t('settings.appearance.rows.editFile'),

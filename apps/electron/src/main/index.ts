...
import { validateGitBashPath, checkVCRedistInstalled } from '@zhangyuge-agent/server-core/services'
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (!isClientOnly) {
      ...
      // Sets env vars so renderer can show an actionable toast with install button.
      if (process.platform === 'win32') {
        const vcCheck = checkVCRedistInstalled()
        if (!vcCheck.installed) {
          mainLog.warn('[vcredist]', vcCheck.message)
          process.env.ZHANGYUGE_AGENT_VCREDIST_MISSING = '1'
          if (vcCheck.downloadUrl) {
            process.env.ZHANGYUGE_AGENT_VCREDIST_URL = vcCheck.downloadUrl
          }
        } else if (isDebugMode) {

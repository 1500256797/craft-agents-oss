 ...
 * Deep Link Handler
 *
 * Parses zhangyuge-agent:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   zhangyuge-agent://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   zhangyuge-agent://flagged[/session/{sessionId}]             - Session list (flagged filter)
 ...
 *   zhangyuge-agent://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   zhangyuge-agent://allSessions/session/abc123                (specific session)
 *   zhangyuge-agent://settings/shortcuts                     (shortcuts page)
 *   zhangyuge-agent://sources/source/github                  (github source info)
 *   zhangyuge-agent://action/new-chat                        (uses active window)
 *   zhangyuge-agent://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   zhangyuge-agent://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., zhangyuge-agent://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., zhangyuge-agent://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // zhangyuge-agent://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // zhangyuge-agent://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // zhangyuge-agent://action/... (no workspace - uses active window)
    if (host === 'action') {

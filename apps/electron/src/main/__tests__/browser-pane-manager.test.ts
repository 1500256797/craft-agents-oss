...
describe('BrowserPaneManager', () => {
  ...
  it('denies app deep-link popups and forwards to deep-link handler', async () => {
    ...
    await Bun.sleep(0)
    expect(mockShellOpenExternal).toHaveBeenCalledWith('zhangyuge-agent://settings')
  })
  ...
  it('navigate treats plain text as search query', async () => {
    ...
    await manager.navigate('nav-2', 'zhangyuge agents browser tools')
    const instance = (manager as any).instances.get('nav-2')
    expect(instance.pageView.webContents.loadURL).toHaveBeenCalledWith(
      'https://duckduckgo.com/?q=zhangyuge%20agents%20browser%20tools'
    )
  })
  ...
  it('applies observer theme signal and skips regular console logging for it', () => {
    ...
    instance.pageView.webContents._emit('console-message', 1, '__zhangyuge_theme_color__:tok-1:#123456')
  ...
  it('dedupes repeated observer theme signals', () => {
    ...
    instance.pageView.webContents._emit('console-message', 1, '__zhangyuge_theme_color__:tok-2:#445566')
  ...
  it('ignores observer theme signals from stale token', () => {
    ...
    instance.pageView.webContents._emit('console-message', 1, '__zhangyuge_theme_color__:tok-old:#bbccdd')
  ...
  it('clears theme on explicit null sentinel signal', () => {
    ...
    instance.pageView.webContents._emit('console-message', 1, '__zhangyuge_theme_color__:tok-null:#223344')
    expect(manager.listInstances().find(i => i.id === 'theme-null')?.themeColor).toBe('#223344')

    instance.pageView.webContents._emit('console-message', 1, '__zhangyuge_theme_color__:tok-null:__NULL__')
  ...
  it('replays full toolbar state when toolbar renderer finishes loading', () => {
    ...
    instance.currentUrl = 'https://zhangyuge-agent.local'
    instance.title = '章鱼哥AI'
    ...
    expect(sendCallsAfterFinishLoad).toContainEqual([
      'browser-toolbar:state-update',
      {
        url: 'https://zhangyuge-agent.local',
        title: '章鱼哥AI',
        isLoading: true,
        canGoBack: true,
        canGoForward: true,

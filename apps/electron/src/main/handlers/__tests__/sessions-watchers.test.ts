...
import { registerSessionsHandlers, cleanupSessionFileWatchForClient } from '@zhangyuge-agent/server-core/handlers/rpc'
...
describe('sessions file watchers', () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = mkdtempSync(join(tmpdir(), 'zhangyuge-agent-session-watchers-'))

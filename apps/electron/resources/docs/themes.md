...
This guide explains how to customize the visual theme of 章鱼哥AI.

## Overview

章鱼哥AI uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.zhangyuge-agent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.zhangyuge-agent/theme.json` - Override specific colors (app-level)
...
```
~/.zhangyuge-agent/workspaces/{id}/config.json
```
...
Create `~/.zhangyuge-agent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.zhangyuge-agent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.zhangyuge-agent/themes/{name}.json`
...
1. Create `~/.zhangyuge-agent/theme.json` for overrides or `~/.zhangyuge-agent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.zhangyuge-agent/theme.json` for overrides, `~/.zhangyuge-agent/themes/` for presets)

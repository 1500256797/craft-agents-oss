...
This guide explains how to create and configure skills in 章鱼哥AI.

> **CLI-first workflow (recommended):** Use `zhangyuge-agent skill ...` commands instead of editing `SKILL.md` files directly.
...
章鱼哥AI uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in 章鱼哥AI
...
1. **Workspace skill checked first** - If `~/.zhangyuge-agent/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.zhangyuge-agent/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.zhangyuge-agent/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.zhangyuge-agent/workspaces/{ws}/skills/commit/SKILL.md`

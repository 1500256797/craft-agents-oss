...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    const prevUrl = process.env.ZHANGYUGE_AGENT_SERVER_URL
    const prevToken = process.env.ZHANGYUGE_AGENT_SERVER_TOKEN
    const prevCa = process.env.ZHANGYUGE_AGENT_TLS_CA

    process.env.ZHANGYUGE_AGENT_SERVER_URL = 'ws://env-server:8080'
    process.env.ZHANGYUGE_AGENT_SERVER_TOKEN = 'env-token'
    process.env.ZHANGYUGE_AGENT_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevUrl === undefined) delete process.env.ZHANGYUGE_AGENT_SERVER_URL
      else process.env.ZHANGYUGE_AGENT_SERVER_URL = prevUrl
      if (prevToken === undefined) delete process.env.ZHANGYUGE_AGENT_SERVER_TOKEN
      else process.env.ZHANGYUGE_AGENT_SERVER_TOKEN = prevToken
      if (prevCa === undefined) delete process.env.ZHANGYUGE_AGENT_TLS_CA
      else process.env.ZHANGYUGE_AGENT_TLS_CA = prevCa
    }
  })
  ...
  it('--source accumulates into array', () => {
    ...
    expect(args.sources).toEqual(['zhangyuge-kb', 'github'])
  })

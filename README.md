...
## How it Works (Video)
To understand what 章鱼哥AI does and how it works watch this video.
...
## Why 章鱼哥AI was built
章鱼哥AI is a tool we built so that we (at zhangyuge-agent.local) can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or Service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
章鱼哥AI is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building 章鱼哥AI with 章鱼哥AI only - no code editors - so really, any customisation is just a prompt away.

We built 章鱼哥AI because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
[Check out how I just connected to Slack →](https://agents.zhangyuge-agent.local/s/DRNQEiy8w2e1v5LPgKl8b)
...
章鱼哥AI connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
[Here I imported all my skills in one go →](https://agents.zhangyuge-agent.local/s/gWCFqwhObFWaNJIEJmd6j)
...
**macOS / Linux:**
```bash
curl -fsSL https://agents.zhangyuge-agent.local/install-app.sh | bash
```

**Windows (PowerShell):**
```powershell
irm https://agents.zhangyuge-agent.local/install-app.ps1 | iex
```

### Build from Source

```bash
git clone https://github.com/lukilabs/zhangyuge-agent.git
cd zhangyuge-agent
bun install
...
- **章鱼哥AI MCP Integration**: Access to 32+ 章鱼哥AI document tools (blocks, collections, search, tasks)
...
| Type | Examples |
|------|----------|
| **MCP Servers** | 章鱼哥AI, Linear, GitHub, Notion, custom servers |
...
章鱼哥AI can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
ZHANGYUGE_AGENT_SERVER_TOKEN=$(openssl rand -hex 32) bun run packages/server/src/index.ts
```

The server prints the connection details on startup:

```
ZHANGYUGE_AGENT_SERVER_URL=ws://203.0.113.5:9100
ZHANGYUGE_AGENT_SERVER_TOKEN=<generated-token>
```
...
```bash
ZHANGYUGE_AGENT_SERVER_URL=wss://203.0.113.5:9100 ZHANGYUGE_AGENT_SERVER_TOKEN=<token> bun run electron:start
```
...
|----------|----------|---------|-------------|
| `ZHANGYUGE_AGENT_SERVER_TOKEN` | Yes | — | Bearer token for client authentication |
| `ZHANGYUGE_AGENT_RPC_HOST` | No | `127.0.0.1` | Bind address (`0.0.0.0` for remote access) |
| `ZHANGYUGE_AGENT_RPC_PORT` | No | `9100` | Bind port |
| `ZHANGYUGE_AGENT_RPC_TLS_CERT` | No | — | Path to PEM certificate file (enables `wss://`) |
| `ZHANGYUGE_AGENT_RPC_TLS_KEY` | No | — | Path to PEM private key file (required with cert) |
| `ZHANGYUGE_AGENT_RPC_TLS_CA` | No | — | Path to PEM CA chain file (optional, for client cert verification) |
| `ZHANGYUGE_AGENT_DEBUG` | No | `false` | Enable debug logging |
...
ZHANGYUGE_AGENT_RPC_HOST=0.0.0.0 \
ZHANGYUGE_AGENT_RPC_TLS_CERT=certs/cert.pem \
ZHANGYUGE_AGENT_RPC_TLS_KEY=certs/key.pem \
bun run packages/server/src/index.ts
```

The server will print `ZHANGYUGE_AGENT_SERVER_URL=wss://<your-public-ip>:9100`.
...
docker run -d \
  -p 9100:9100 \
  -e ZHANGYUGE_AGENT_SERVER_TOKEN=<token> \
  -e ZHANGYUGE_AGENT_RPC_HOST=0.0.0.0 \
  -v zhangyuge-agent-data:/root/.zhangyuge-agent \
...
docker run -d \
  ...
  -e ZHANGYUGE_AGENT_SERVER_TOKEN=<token> \
  -e ZHANGYUGE_AGENT_RPC_HOST=0.0.0.0 \
  -e ZHANGYUGE_AGENT_RPC_TLS_CERT=/certs/cert.pem \
  -e ZHANGYUGE_AGENT_RPC_TLS_KEY=/certs/key.pem \
  -v ./certs:/certs:ro \
  -v zhangyuge-agent-data:/root/.zhangyuge-agent \
  zhangyuge-agent-server
```
...
A terminal client that connects to a running 章鱼哥AI server over WebSocket (`ws://` or `wss://`). Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
# Or add to your PATH
alias zhangyuge-agent-cli="bun run $(pwd)/apps/cli/src/index.ts"
```
...
export ZHANGYUGE_AGENT_SERVER_URL=ws://127.0.0.1:9100
export ZHANGYUGE_AGENT_SERVER_TOKEN=<your-token>

# Or via flags
zhangyuge-agent-cli --url ws://127.0.0.1:9100 --token <token> ping
```
...
zhangyuge-agent-cli ping

# List sessions (human-readable)
zhangyuge-agent-cli sessions

# Send a message and stream the AI response
zhangyuge-agent-cli send abc-123 "What files are in the current directory?"

# Pipe input
echo "Summarize this" | zhangyuge-agent-cli send abc-123

# JSON output for scripting
zhangyuge-agent-cli --json workspaces | jq '.[].name'

# Self-contained run (spawns its own server)
zhangyuge-agent-cli run "Summarize the README"
zhangyuge-agent-cli run --workspace-dir ./my-project --source github "List open PRs"

# Multi-provider support
zhangyuge-agent-cli run --provider openai --model gpt-4o "Summarize this repo"
GOOGLE_API_KEY=... zhangyuge-agent-cli run --provider google --model gemini-2.0-flash "Hello"
zhangyuge-agent-cli run --provider anthropic --base-url https://openrouter.ai/api/v1 --api-key $OR_KEY "Hello"

# Validate the server (auto-spawns if no --url)
zhangyuge-agent-cli --validate-server
zhangyuge-agent-cli --validate-server --url ws://127.0.0.1:9100 --token <token>
```
...
└── packages/
    ...
    └── shared/                # Business logic
        └── src/
            ├── agent/         # ZhangyugeAgent, permissions
            ├── auth/          # OAuth, tokens
...
# Debug logging (writes to ~/Library/Logs/@zhangyuge-agent/electron/)
...
4. Name: e.g., "章鱼哥AI Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in 章鱼哥AI
...
章鱼哥AI supports multiple ways to connect to LLM providers:
...
Configuration is stored at `~/.zhangyuge-agent/`:
...
Or configure manually in `~/.zhangyuge-agent/workspaces/{id}/automations.json`:
...
**Prompt actions** create a new agent session with a prompt. They support `@mentions` for sources and skills, and environment variables like `$ZHANGYUGE_AGENT_LABEL` and `$ZHANGYUGE_AGENT_SESSION_ID` are expanded automatically.
...
See the [Automations documentation](https://agents.zhangyuge-agent.local/docs/automations/overview) for the full reference.
...
External apps can navigate using `zhangyuge-agent://` URLs:

```
zhangyuge-agent://allSessions                      # All sessions view
zhangyuge-agent://allSessions/session/session123   # Specific session
zhangyuge-agent://settings                         # Settings
zhangyuge-agent://sources/source/github            # Source info
zhangyuge-agent://action/new-chat                  # Create new session
```
...
**macOS:**
```bash
/Applications/章鱼哥AI.app/Contents/MacOS/章鱼哥AI -- --debug
```

**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\@zhangyuge-agentelectron\章鱼哥AI.exe" -- --debug
```

**Linux:**
```bash
./zhangyuge-agent -- --debug
```

Logs are written to:
- **macOS:** `~/Library/Logs/@zhangyuge-agent/electron/main.log`
- **Windows:** `%APPDATA%\@zhangyuge-agent\electron\logs\main.log`
- **Linux:** `~/.config/@zhangyuge-agent/electron/logs/main.log`
...
"章鱼哥AI" is a trademark of Zhangyuge Agent. See [TRADEMARK.md](TRADEMARK.md) for usage guidelines.

# zhangyuge-agent-cli — CLI Reference

Terminal client for 章鱼哥AI server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running 章鱼哥AI headless server with URL and token

## Installation

```bash
# Clone the repository
git clone https://github.com/anthropics/zhangyuge-agent.git
cd zhangyuge-agent
...
# Option B: Link globally (adds zhangyuge-agent-cli to PATH)
cd apps/cli && bun link
zhangyuge-agent-cli <command>
```
...
|------|---------|---------|-------------|
| `--url <ws[s]://...>` | `ZHANGYUGE_AGENT_SERVER_URL` | — | Server WebSocket URL |
| `--token <secret>` | `ZHANGYUGE_AGENT_SERVER_TOKEN` | — | Authentication token |
| `--workspace <id>` | — | auto-detect | Workspace ID |
| `--timeout <ms>` | — | `10000` | Request timeout |
| `--tls-ca <path>` | `ZHANGYUGE_AGENT_TLS_CA` | — | Custom CA cert for self-signed TLS |
...
```bash
zhangyuge-agent-cli ping              # Verify connectivity (clientId + latency)
zhangyuge-agent-cli health            # Check credential store health
zhangyuge-agent-cli versions          # Show server runtime versions
```

### Resource Listing

```bash
zhangyuge-agent-cli workspaces        # List all workspaces
zhangyuge-agent-cli sessions          # List sessions in workspace
zhangyuge-agent-cli connections       # List LLM connections
zhangyuge-agent-cli sources           # List configured sources
```

### Session Operations

```bash
zhangyuge-agent-cli session create [--name <n>] [--mode <m>]  # Create session
zhangyuge-agent-cli session messages <id>                       # Print message history
zhangyuge-agent-cli session delete <id>                         # Delete session
zhangyuge-agent-cli cancel <id>                                 # Cancel processing
```
...
zhangyuge-agent-cli send <session-id> <message>

# Pipe text from stdin
echo "Summarize this file" | zhangyuge-agent-cli send <session-id>

# Read from stdin explicitly
cat document.txt | zhangyuge-agent-cli send <session-id> --stdin
```
...
zhangyuge-agent-cli invoke <channel> [json-args...]

# Subscribe to push events (Ctrl+C to stop)
zhangyuge-agent-cli listen <channel>
```

Examples:
```bash
zhangyuge-agent-cli invoke system:homeDir
zhangyuge-agent-cli invoke sessions:get '"workspace-123"'
zhangyuge-agent-cli listen session:event
```

### Run (Self-Contained)

```bash
zhangyuge-agent-cli run <prompt>
zhangyuge-agent-cli run --workspace-dir ./project --source github "List open PRs"
```
...
```bash
echo "Summarize this file" | zhangyuge-agent-cli run
cat error.log | zhangyuge-agent-cli run "What's causing these errors?"
```
...
```bash
# Get workspace IDs
WORKSPACES=$(zhangyuge-agent-cli --json workspaces | jq -r '.[].id')

# Count sessions per workspace
for ws in $WORKSPACES; do
  COUNT=$(zhangyuge-agent-cli --json --workspace "$ws" sessions | jq length)
  echo "$ws: $COUNT sessions"
done

# Create a session and capture its ID
SESSION_ID=$(zhangyuge-agent-cli --json session create --name "CI Run" | jq -r '.id')

# Send a message and wait for completion
zhangyuge-agent-cli send "$SESSION_ID" "Run the test suite and report results"

# Clean up
zhangyuge-agent-cli session delete "$SESSION_ID"
```
...
zhangyuge-agent-cli --url wss://server.example.com:9100 ping

# Self-signed certificate
zhangyuge-agent-cli --url wss://server.example.com:9100 --tls-ca /path/to/ca.pem ping
```

The `--tls-ca` flag sets `NODE_EXTRA_CA_CERTS` before connecting. You can also set `ZHANGYUGE_AGENT_TLS_CA` in your environment.
...
| `AUTH_FAILED` | Wrong token | Check `ZHANGYUGE_AGENT_SERVER_TOKEN` matches server |

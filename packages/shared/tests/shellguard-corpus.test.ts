...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // zhangyuge-agent CLI read-only
    { regex: /^zhangyuge-agent\s+label\s+(list|get)\b/, source: '^zhangyuge-agent\\s+label\\s+(list|get)\\b', comment: 'zhangyuge-agent label read-only operations' },
    { regex: /^zhangyuge-agent\s+source\s+(list|get|validate|test)\b/, source: '^zhangyuge-agent\\s+source\\s+(list|get|validate|test)\\b', comment: 'zhangyuge-agent source read-only operations' },
    { regex: /^zhangyuge-agent\s+skill\s+(list|get|validate|where)\b/, source: '^zhangyuge-agent\\s+skill\\s+(list|get|validate|where)\\b', comment: 'zhangyuge-agent skill read-only operations' },
    { regex: /^zhangyuge-agent\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^zhangyuge-agent\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'zhangyuge-agent automation read-only operations' },
    { regex: /^zhangyuge-agent\s*$/, source: '^zhangyuge-agent\\s*$', comment: 'zhangyuge-agent bare invocation' },
    { regex: /^zhangyuge-agent\s+(label|source|skill|automation)\s+--help\b/, source: '^zhangyuge-agent\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'zhangyuge-agent entity help flags' },
    { regex: /^zhangyuge-agent\s+--(help|version|discover)\b/, source: '^zhangyuge-agent\\s+--(help|version|discover)\\b', comment: 'zhangyuge-agent global flags' },
...
// ============================================================
// Group 27: zhangyuge-agent CLI allowlist
// ============================================================

describe('ShellGuard corpus: zhangyuge-agent CLI allowlist', () => {
  const shouldAllow = [
    'zhangyuge-agent',
    'zhangyuge-agent --help',
    'zhangyuge-agent --version',
    'zhangyuge-agent --discover',
    'zhangyuge-agent label --help',
    ...
    'zhangyuge-agent skill --help',
    'zhangyuge-agent skill list',
    'zhangyuge-agent skill get commit-helper',
    'zhangyuge-agent skill where commit-helper',
    'zhangyuge-agent skill validate commit-helper',
    'zhangyuge-agent automation list',
    'zhangyuge-agent automation get abc123',
    'zhangyuge-agent automation validate',
    'zhangyuge-agent automation history abc123 --limit 5',
    'zhangyuge-agent automation last-executed abc123',
    'zhangyuge-agent automation test abc123 --match "x"',
    'zhangyuge-agent automation lint',
  ];

  const shouldBlock = [
    'zhangyuge-agent label create --name Bug',
    'zhangyuge-agent label update bug --name "Bug Report"',
    'zhangyuge-agent label delete bug',
    'zhangyuge-agent label move bug --parent root',
    'zhangyuge-agent label reorder --parent root a b c',
    'zhangyuge-agent source create --name Linear --provider linear --type mcp',
    'zhangyuge-agent source update linear --json "{\"enabled\":false}"',
    'zhangyuge-agent source delete linear',
    'zhangyuge-agent skill create --name "Review" --description "x"',
    'zhangyuge-agent skill update review --json "{\"description\":\"y\"}"',
    'zhangyuge-agent skill delete review',
    'zhangyuge-agent automation create --event UserPromptSubmit --prompt "x"',
    'zhangyuge-agent automation update abc123 --json "{\"enabled\":false}"',
    'zhangyuge-agent automation delete abc123',
    'zhangyuge-agent automation enable abc123',
    'zhangyuge-agent automation disable abc123',

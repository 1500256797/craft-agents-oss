 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.zhangyuge-agent/workspaces/xxx)
 ...
 * (like zhangyuge-agent-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only zhangyuge-agent-docs is a builtin source
      if (slug === 'zhangyuge-agent-docs') {
 ...
 * Built-in sources (like zhangyuge-agent-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search zhangyuge-agent-docs MCP for service-specific guidance)

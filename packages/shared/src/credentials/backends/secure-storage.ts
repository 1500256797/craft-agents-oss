 ...
 *
 * Stores credentials in an encrypted file at ~/.zhangyuge-agent/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.zhangyuge-agent');
const CREDENTIALS_FILE = join(CREDENTIALS_DIR, 'credentials.enc');

// File format constants
const MAGIC_BYTES = Buffer.from('ZYGAI01\0');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('zhangyuge-agent-v2') // Bumped version for new key derivation
      .digest();

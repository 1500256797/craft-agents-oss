#!/usr/bin/env bun
/**
 * Debug script to print the full 章鱼哥AI system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    ZHANGYUGE AGENT SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. 章鱼哥AI Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.zhangyuge-agent/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@zhangyuge-agent/electron/main.log' },
  '/Users/example/.zhangyuge-agent/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.zhangyuge-agent/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by ZhangyugeAgent.formatSourceState() - requires agent instance');
...
const exampleSkills = `<skills>
  <skill>
    ...
    <description>Helps write conventional commit messages.</description>
  </skill>
  <skill>
    <name>Debugger</name>
    <description>Diagnoses bugs, errors, and unexpected behavior.</description>
  </skill>
</skills>`;
printSection(
  '4. SKILLS CONTEXT - formatSkillsContext()',
  exampleSkills,
  colors.magenta
);
printAnnotation('Generated from global/workspace/project skills discovered on disk');
printAnnotation('Includes only name + description to keep prompt size compact');
...
printSection(
  '5. WORKSPACE CAPABILITIES - formatWorkspaceCapabilities()',
  exampleCapabilities,
  colors.magenta
);
...
printSection(
  '6. WORKING DIRECTORY - getWorkingDirectoryContext()',
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// 章鱼哥AI additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. 章鱼哥AI Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Dynamic User Message Components (per message):${colors.reset}
  ...
  4. Skills Context                      ${colors.dim}// formatSkillsContext()${colors.reset}
  5. Workspace Capabilities              ${colors.dim}// formatWorkspaceCapabilities()${colors.reset}
  6. Working Directory + project_context_file  ${colors.dim}// getWorkingDirectoryContext()${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/core/prompt-builder.ts ${colors.dim}// Dynamic context building${colors.reset}
  packages/shared/src/agent/base-agent.ts        ${colors.dim}// Message preprocessing + skill directives${colors.reset}

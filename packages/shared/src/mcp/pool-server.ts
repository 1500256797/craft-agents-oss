...
export class McpPoolServer {
   ...
   * Tools from pool use `mcp__zhangyuge__search_spaces` naming internally.
   ...
   *   pool internal: mcp__zhangyuge__search_spaces
   *   exposed here:  zhangyuge__search_spaces
   *   Codex sees:    mcp__sources__zhangyuge__search_spaces
   */
  private createMcpServer(): Server {
    const server = new Server(
      { name: 'zhangyuge-agent-pool-proxy', version: '1.0.0' },
      { capabilities: { tools: {} } }
    );

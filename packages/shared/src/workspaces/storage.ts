 ...
 * Default location: ~/.zhangyuge-agent/workspaces/
 */
...
const CONFIG_DIR = join(homedir(), '.zhangyuge-agent');
...
/**
 * Get the default workspaces directory (~/.zhangyuge-agent/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.zhangyuge-agent/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.zhangyuge-agent/workspaces/
 */
 ...
 * Resolution order: ENV (ZHANGYUGE_AGENT_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.ZHANGYUGE_AGENT_LOCAL_MCP_ENABLED;
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `zhangyuge-agent-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };

...
import { getSessionSafeAllowedToolNames } from '@zhangyuge-agent/session-tools-core';
...
class ModeManager {
  ...
  setPermissionMode(
    ...
    // Notify callbacks (for ZhangyugeAgent internal sync)
...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.zhangyuge-agent/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .zhangyuge-agent entirely
  if (!normalizedTarget.includes('/.zhangyuge-agent/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__zhangyuge-agent-docs__')) {
      return { allowed: true };
    }

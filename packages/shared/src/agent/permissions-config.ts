 ...
 *
 * File locations:
 * - Workspace: ~/.zhangyuge-agent/workspaces/{slug}/permissions.json
 * - Per-source: ~/.zhangyuge-agent/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.zhangyuge-agent/permissions/
 * Reads env var dynamically so tests can override via ZHANGYUGE_AGENT_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.ZHANGYUGE_AGENT_CONFIG_DIR || join(homedir(), '.zhangyuge-agent');
...
/**
 * Load default permissions from ~/.zhangyuge-agent/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.zhangyugeAgentCli && pattern.startsWith('^zhangyuge-agent\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.zhangyuge-agent/permissions/default.json)
   ...
   * These come from ~/.zhangyuge-agent/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping zhangyuge-agent bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }

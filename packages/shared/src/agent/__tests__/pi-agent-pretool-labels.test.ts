...
beforeAll(() => {
  origFlag = process.env.ZHANGYUGE_AGENT_FEATURE_CLI
...
afterAll(() => {
  if (origFlag === undefined) {
    ...
    process.env.ZHANGYUGE_AGENT_FEATURE_CLI = origFlag
  }
})
...
describe('PiAgent pre-tool labels guard', () => {
  it('blocks Read on workspace labels/config.json even when session workingDirectory is outside workspace root', async () => {
    ...
    expect(String(response?.reason ?? '')).toContain('zhangyuge-agent label --help')

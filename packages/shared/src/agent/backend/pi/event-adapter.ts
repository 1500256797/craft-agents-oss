 ...
 * 章鱼哥AI's AgentEvent format for UI compatibility.
 *
...
import type { AgentEvent as ZhangyugeAgentEvent } from '@zhangyuge-agent/core/types';
import type {
...
/**
 * Maps Pi SDK events to 章鱼哥AIEvents for UI compatibility.
 *
 * Event mapping:
...
export class PiEventAdapter extends BaseEventAdapter {
  ...
  /**
   * Adapt a Pi SDK event to zero or more 章鱼哥AIEvents.
   */
  *adaptEvent(event: PiEvent): Generator<ZhangyugeAgentEvent> {
    switch (event.type) {
      // ============================================================

...
let mockZhangyugeAgentCliFlag = false;
mock.module('../../../feature-flags.ts', () => ({
  FEATURE_FLAGS: {
    get zhangyugeAgentCli() {
      return mockZhangyugeAgentCliFlag;
    },
...
describe('runPreToolUseChecks', () => {
  beforeEach(() => {
    ...
    mockZhangyugeAgentCliFlag = false;
  });
  ...
  describe('step 2: source blocking', () => {
    ...
    it('skips source check for built-in MCP servers (zhangyuge-agent-docs)', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'mcp__zhangyuge-agent-docs__search',
        input: {},
        activeSourceSlugs: [],
      }));
  ...
  describe('step 5: input transforms', () => {
    beforeEach(() => {
      mockZhangyugeAgentCliFlag = true;
    });
    ...
    it('blocks direct label folder reads and suggests zhangyuge-agent label help when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('zhangyuge-agent label');
        expect(result.reason).toContain('zhangyuge-agent label --help');
        expect(result.reason).toContain('labels/');
      }
    });

    it('blocks direct label config writes and suggests zhangyuge-agent label help when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
    ...
    it('blocks direct automations config edits and suggests zhangyuge-agent automation commands when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('zhangyuge-agent automation');
        expect(result.reason).toContain('automations.json');
      }
    });

    it('blocks direct source config edits and suggests zhangyuge-agent source commands when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('zhangyuge-agent source');
        expect(result.reason).toContain('sources/linear/config.json');
      }
    });

    it('blocks direct skill file edits and suggests zhangyuge-agent skill commands when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
      mockDetectConfigFileType.mockImplementation(() => ({
        type: 'skill',
        slug: 'commit-helper',
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('zhangyuge-agent skill');
        expect(result.reason).toContain('skills/commit-helper/SKILL.md');
      }
    });

    it('blocks bash commands touching labels paths and points to zhangyuge-agent label --help when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
    ...
    it('allows bash zhangyuge-agent label commands through labels guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'zhangyuge-agent label list' },
        permissionMode: 'allow-all',
      }));
    ...
    it('blocks bash commands touching automations files and points to zhangyuge-agent automation --help when feature is enabled', () => {
      mockZhangyugeAgentCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('zhangyuge-agent automation --help');
        expect(result.reason).toContain('zhangyuge-agent automation');
      }
    });

    it('allows bash zhangyuge-agent automation commands through config-domain bash guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'zhangyuge-agent automation list' },
        permissionMode: 'allow-all',
      }));

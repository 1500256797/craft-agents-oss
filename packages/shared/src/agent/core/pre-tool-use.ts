...
import {
  CLI_DOMAIN_POLICIES,
  ZHANGYUGE_AGENT_CLI_BASH_GUARD_SCOPE_ENTRIES,
  ZHANGYUGE_AGENT_CLI_WORKSPACE_SCOPE_ENTRIES,
...
export function qualifySkillName(
  ...
  // Extract the bare slug — strip any existing qualifier (e.g. "ZhangyugeAgentWS:commit" → "commit")
...
function buildCliDomainBlockMessage(namespace: CliDomainNamespace, context: string): string {
  ...
  return [
    `${context}`,
    `Use \`zhangyuge-agent ${namespace} ...\` instead.`,
...
export function getConfigCliRedirect(
  ...
  if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
    ...
    if (relativePath) {
      const labelsScopeMatch = ZHANGYUGE_AGENT_CLI_WORKSPACE_SCOPE_ENTRIES.find(
...
/**
 * Block bash commands that operate on guarded config paths unless they use zhangyuge-agent commands.
...
export function getConfigDomainBashRedirect(
  ...
  if (/^zhangyuge-agent\s+(label|automation|source|skill)\b/.test(command)) {
    return null;
  }
  ...
  const bashGuardEntries: Array<{ namespace: CliDomainNamespace; scope: string }> = ZHANGYUGE_AGENT_CLI_BASH_GUARD_SCOPE_ENTRIES
...
const BUILT_IN_MCP_SERVERS = new Set(['session', 'zhangyuge-agent-docs']);
...
export function runPreToolUseChecks(ctx: PreToolUseInput): PreToolUseCheckResult {
  ...
  // 5b. Config-domain Bash guard (block direct labels/automations path operations unless using zhangyuge-agent)
  if (FEATURE_FLAGS.zhangyugeAgentCli && toolName === 'Bash') {
  ...
  if (FEATURE_FLAGS.zhangyugeAgentCli) {

...
function buildApiDetails(context: ClaudeSdkErrorContext): string[] {
  ...
  if (context.actualError?.message) add(`Error: ${context.actualError.message}`);
  if (context.actualError?.errorType) add(`Type: ${context.actualError.errorType}`);
  if (context.actualError?.requestId) add(`Request ID: ${context.actualError.requestId}`);
...
function classifyFailure(errorCode: SDKAssistantMessageError, context: ClaudeSdkErrorContext): FailureKind {
  ...
  if (hasProviderStatus || hasProviderType || hasProviderText) return 'provider';
  if (hasNetworkText) return 'network';
  return 'unknown';
}
...
export function mapClaudeSdkAssistantError(
  ...
  const providerError: AgentError = {
    ...
    details: [...apiDetails, 'Your credentials and local setup may still be correct.'],
    actions: retryAction,
    canRetry: true,
    retryDelayMs: 5000,
  };
  ...
  const networkError: AgentError = {
    ...
    details: [...apiDetails, 'Verify your network connection is active', 'Firewall or VPN may be blocking the connection'],
    actions: retryAction,
    canRetry: true,
    retryDelayMs: 2000,
  };
  ...
  switch (errorCode) {
    ...
    case 'invalid_request':
      return {
        ...
        details: [...apiDetails, 'Try removing any attachments and resending', 'Check if images are in a supported format (PNG, JPEG, GIF, WebP)'],
        actions: retryAction,
        canRetry: true,
        retryDelayMs: 1000,
      };
    ...
    case 'unknown':
      if (failureKind === 'provider') return providerError;
      if (failureKind === 'network') return networkError;
      return {
        code: 'unknown_error',
        title: 'Unknown Error',
        message: 'An unexpected error occurred.',
        details: [...apiDetails, 'This may be a temporary issue', 'Check your network connection'],
        actions: retryAction,
        canRetry: true,
        retryDelayMs: 2000,
      };

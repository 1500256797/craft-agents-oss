 ...
 * Guides are now served exclusively via the zhangyuge-agent-docs MCP server.
 *
...
export function extractDomainFromSource(source: {
  ...
  if (source.provider) {
    ...
    const providerDomains: Record<string, string> = {
      ...
      notion: 'notion.so',
      slack: 'slack.com',
      'zhangyuge-agent': 'zhangyuge-agent.local',
      exa: 'exa.ai',
      google: 'google.com',
    };
 ...
 * Use the zhangyuge-agent-docs MCP server to search for setup guides.
 *
 * Example: mcp__zhangyuge-agent-docs__SearchZhangyugeAgentDocs({ query: "github source setup guide" })
 */

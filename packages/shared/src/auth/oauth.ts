...
export interface OAuthConfig {
  mcpUrl: string; // Full MCP URL including path (e.g., https://mcp.zhangyuge-agent.local/my/mcp)
}
 ...
 * Extracted from 章鱼哥AIOAuth.registerClient for reuse in prepareMcpOAuth.
 */
...
export async function prepareMcpOAuth(mcpUrl: string, callbackPort: number): Promise<PreparedOAuthFlow> {
  ...
  if (metadata.registration_endpoint) {
    try {
      ...
      // Fall back to the default public client ID when registration is forbidden.
      clientId = 'zhangyuge-agent';
    }
  } else {
    clientId = 'zhangyuge-agent';
...
export async function discoverOAuthMetadata(
  ...
  // 1. Try RFC 9728 protected resource discovery first (handles 章鱼哥AI MCP and other compliant servers)

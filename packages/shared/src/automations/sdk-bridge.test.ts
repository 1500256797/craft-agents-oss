...
describe('sdk-bridge', () => {
  describe('buildEnvFromSdkInput', () => {
    it('should always include ZHANGYUGE_AGENT_EVENT', () => {
      const env = buildEnvFromSdkInput('PreToolUse', input());
      expect(env.ZHANGYUGE_AGENT_EVENT).toBe('PreToolUse');
    });
    ...
    describe('PreToolUse / PostToolUse', () => {
      it('should map tool_name to ZHANGYUGE_AGENT_TOOL_NAME', () => {
        const env = buildEnvFromSdkInput('PreToolUse', input({ tool_name: 'Bash' }));
        expect(env.ZHANGYUGE_AGENT_TOOL_NAME).toBe('Bash');
      });
      ...
      it('should map tool_input as sanitized JSON', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_TOOL_INPUT).toBeDefined();
        expect(env.ZHANGYUGE_AGENT_TOOL_INPUT).not.toContain('`');
      });
      ...
      it('should map tool_response for PostToolUse', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_TOOL_RESPONSE).toBeDefined();
      });
    });

    describe('PostToolUseFailure', () => {
      it('should map error to ZHANGYUGE_AGENT_ERROR', () => {
        const env = buildEnvFromSdkInput('PostToolUseFailure', input({
          tool_name: 'Bash',
          error: 'Command failed',
        }));
        expect(env.ZHANGYUGE_AGENT_TOOL_NAME).toBe('Bash');
        expect(env.ZHANGYUGE_AGENT_ERROR).toBeDefined();
      });
    });
    ...
    describe('UserPromptSubmit', () => {
      it('should sanitize user prompt', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_PROMPT).toBeDefined();
        // Backticks should be escaped with backslash
        expect(env.ZHANGYUGE_AGENT_PROMPT).toContain('\\`');
      });
    });
    ...
    describe('SessionStart', () => {
      it('should map source and model', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_SOURCE).toBe('manual');
        expect(env.ZHANGYUGE_AGENT_MODEL).toBe('claude-opus-4-6');
      });
    });
    ...
    describe('SubagentStart / SubagentStop', () => {
      it('should map agent_id and agent_type', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_AGENT_ID).toBe('agent-123');
        expect(env.ZHANGYUGE_AGENT_AGENT_TYPE).toBe('research');
      });
    });
    ...
    describe('Notification', () => {
      it('should sanitize message and title', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_MESSAGE).toBeDefined();
        expect(env.ZHANGYUGE_AGENT_TITLE).toBeDefined();
        // Backticks should be escaped
        expect(env.ZHANGYUGE_AGENT_MESSAGE).toContain('\\`');
        expect(env.ZHANGYUGE_AGENT_TITLE).toContain('\\`');
      });
    });
    ...
    describe('unknown/default events', () => {
      it('should return minimal env for events with no specific mappings', () => {
        ...
        expect(env.ZHANGYUGE_AGENT_EVENT).toBe('Stop');

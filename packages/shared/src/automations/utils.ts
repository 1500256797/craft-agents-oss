...
/**
 * Build the base ZHANGYUGE_AGENT_* environment variables shared by both prompt and webhook actions.
...
function buildBaseEventEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  const env: Record<string, string> = {
    ZHANGYUGE_AGENT_EVENT: event,
    ZHANGYUGE_AGENT_EVENT_DATA: JSON.stringify(payload),
  };

  if (payload.sessionId) env.ZHANGYUGE_AGENT_SESSION_ID = payload.sessionId;
  if (payload.sessionName) env.ZHANGYUGE_AGENT_SESSION_NAME = payload.sessionName;
  if (payload.workspaceId) env.ZHANGYUGE_AGENT_WORKSPACE_ID = payload.workspaceId;
  ...
  if (Object.keys(sessionMetadata).length > 0) {
    env.ZHANGYUGE_AGENT_SESSION_METADATA = JSON.stringify(sessionMetadata);
  }

  // Local time for scheduler events
  if (event === 'SchedulerTick') {
    const now = new Date();
    env.ZHANGYUGE_AGENT_LOCAL_TIME = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
    env.ZHANGYUGE_AGENT_LOCAL_DATE = now.toISOString().split('T')[0]!;
  }

  // Payload fields as ZHANGYUGE_AGENT_ vars (raw — callers apply sanitization if needed)
  for (const [key, value] of Object.entries(payload)) {
    if (PAYLOAD_SKIP_KEYS.has(key)) continue;
    const envKey = `ZHANGYUGE_AGENT_${toSnakeCase(key).toUpperCase()}`;
...
export function buildEnvFromPayload(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  if (payload.sessionName) env.ZHANGYUGE_AGENT_SESSION_NAME = sanitizeForShell(payload.sessionName);
 ...
 * - Only injects ZHANGYUGE_AGENT_WH_* user-defined vars from process.env (webhook secrets)
 * - Includes ZHANGYUGE_AGENT_* system vars derived from the event payload
 *
 ...
 *   export ZHANGYUGE_AGENT_WH_DISCORD_TOKEN="abc123"
 *
 * Then reference them in automations.json:
 *   "url": "${ZHANGYUGE_AGENT_WH_SLACK_URL}"
 *   "headers": { "Authorization": "Bearer ${ZHANGYUGE_AGENT_WH_DISCORD_TOKEN}" }
 */
export function buildWebhookEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  // User-defined webhook secrets: only ZHANGYUGE_AGENT_WH_* from process.env
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('ZHANGYUGE_AGENT_WH_') && value !== undefined) {
      env[key] = value;
    }
  }

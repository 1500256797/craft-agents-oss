...
import { loadWorkspaceSources } from '@zhangyuge-agent/shared/sources'
import { safeJsonParse } from '@zhangyuge-agent/shared/utils/files'
...
export function registerSourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Create a new source
  server.handle(RPC_CHANNELS.sources.CREATE, async (_ctx, workspaceId: string, config: Partial<import('@zhangyuge-agent/shared/sources').CreateSourceInput>) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createSource } = await import('@zhangyuge-agent/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.DELETE, async (_ctx, workspaceId: string, sourceSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteSource } = await import('@zhangyuge-agent/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.SAVE_CREDENTIALS, async (_ctx, workspaceId: string, sourceSlug: string, credential: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { loadSource, getSourceCredentialManager } = await import('@zhangyuge-agent/shared/sources')
  ...
  server.handle(RPC_CHANNELS.sources.GET_PERMISSIONS, async (_ctx, workspaceId: string, sourceSlug: string) => {
    ...
    const { getSourcePermissionsPath } = await import('@zhangyuge-agent/shared/agent')
  ...
  server.handle(RPC_CHANNELS.workspace.GET_PERMISSIONS, async (_ctx, workspaceId: string) => {
    ...
    const { existsSync, readFileSync } = await import('fs')
    const { getWorkspacePermissionsPath } = await import('@zhangyuge-agent/shared/agent')
  ...
  // Get default permissions from ~/.zhangyuge-agent/permissions/default.json
  server.handle(RPC_CHANNELS.permissions.GET_DEFAULTS, async () => {
    const { existsSync, readFileSync } = await import('fs')
    const { getAppPermissionsDir } = await import('@zhangyuge-agent/shared/agent')
  ...
  server.handle(RPC_CHANNELS.sources.GET_MCP_TOOLS, async (_ctx, workspaceId: string, sourceSlug: string) => {
    ...
    try {
      ...
      const { 章鱼哥AIMcpClient } = await import('@zhangyuge-agent/shared/mcp')
      let client: InstanceType<typeof 章鱼哥AIMcpClient>
      ...
      const { loadSourcePermissionsConfig, permissionsConfigCache } = await import('@zhangyuge-agent/shared/agent')

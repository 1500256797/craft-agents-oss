...
export function parseTestConnectionError(msg: string): string {
  ...
  if (lower.includes('no api key found for')) {
    return 'Setup could not route credentials to the selected provider. For custom endpoints, select the Custom preset and choose the correct protocol (OpenAI Compatible or Anthropic Compatible).'
  }
...
export function validateSetupTestInput(params: {
  ...
  if (params.provider === 'pi' && hasCustomEndpoint && !params.piAuthProvider) {
    return {
      valid: false,
      error: 'Custom endpoint in 章鱼哥AI Backend mode requires selecting a provider preset. For arbitrary Anthropic-compatible endpoints, use Anthropic API Key mode.',
    }
  }

 ...
 *
 * Validates 章鱼哥AI configuration files.
...
export async function handleConfigValidate(
  ...
  const zhangyugeAgentRoot = join(homedir(), '.zhangyuge-agent');
  ...
  switch (target) {
    case 'config': {
      const result = validateJsonFileHasFields(
        join(zhangyugeAgentRoot, 'config.json'),
        ['workspaces']
      );
      return successResponse(formatValidationResult(result));
    }
    ...
    case 'preferences': {
      const result = validateJsonFileHasFields(
        join(zhangyugeAgentRoot, 'preferences.json'),
        []
      );
      return successResponse(formatValidationResult(result));
    }
    ...
    case 'tool-icons': {
      const result = validateJsonFileHasFields(
        join(zhangyugeAgentRoot, 'tool-icons', 'tool-icons.json'),
        ['version', 'tools']
      );
      return successResponse(formatValidationResult(result));
    }

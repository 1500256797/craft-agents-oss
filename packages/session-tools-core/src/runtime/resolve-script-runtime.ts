...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to ZHANGYUGE_AGENT_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.ZHANGYUGE_AGENT_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.ZHANGYUGE_AGENT_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.ZHANGYUGE_AGENT_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute ZHANGYUGE_AGENT_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute ZHANGYUGE_AGENT_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (ZHANGYUGE_AGENT_UV / ZHANGYUGE_AGENT_NODE / ZHANGYUGE_AGENT_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.ZHANGYUGE_AGENT_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ZHANGYUGE_AGENT_UV, 'Python/uv')
        : process.env.ZHANGYUGE_AGENT_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure ZHANGYUGE_AGENT_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.ZHANGYUGE_AGENT_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ZHANGYUGE_AGENT_NODE, 'Node')
        : process.env.ZHANGYUGE_AGENT_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure ZHANGYUGE_AGENT_NODE or install node on PATH.'
    );
  }

  if (process.env.ZHANGYUGE_AGENT_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.ZHANGYUGE_AGENT_BUN, 'Bun')
      : process.env.ZHANGYUGE_AGENT_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure ZHANGYUGE_AGENT_BUN or install bun on PATH.'
  );
}

# 章鱼哥AI Windows Installer
# Usage: irm https://agents.zhangyuge-agent.local/install-app.ps1 | iex

& {
$ErrorActionPreference = "Stop"

$VERSIONS_URL = "https://agents.zhangyuge-agent.local/electron"
$DOWNLOAD_DIR = "$env:TEMP\zhangyuge-agent-install"
$APP_NAME = "章鱼哥AI"
...
$process = Get-Process -Name "章鱼哥AI" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing 章鱼哥AI..."
...
Write-Info "Adding 'zhangyuge-agent' command to PATH..."

$binDir = "$env:LOCALAPPDATA\章鱼哥AI\bin"
$cmdFile = "$binDir\zhangyuge-agent.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\章鱼哥AI\章鱼哥AI.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'zhangyuge-agent' command)"
} else {
    Write-Success "Command 'zhangyuge-agent' is ready"
}
...
Write-Host ""
Write-Host "  章鱼哥AI has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: zhangyuge-agent (restart terminal first)"
Write-Host ""
}

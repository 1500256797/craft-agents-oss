...
// Detects instance number from folder name suffix (e.g., zhangyuge-agent-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.ZHANGYUGE_AGENT_VITE_PORT) return;
  ...
  if (match) {
    ...
    process.env.ZHANGYUGE_AGENT_INSTANCE_NUMBER = instanceNum;
    process.env.ZHANGYUGE_AGENT_VITE_PORT = `${instanceNum}173`;
    process.env.ZHANGYUGE_AGENT_APP_NAME = `章鱼哥AI [${instanceNum}]`;
    process.env.ZHANGYUGE_AGENT_CONFIG_DIR = join(process.env.HOME || "", `.zhangyuge-agent-${instanceNum}`);
    process.env.ZHANGYUGE_AGENT_DEEPLINK_SCHEME = `zhangyuge-agent${instanceNum}`;
    console.log(`🔢 Instance ${instanceNum} detected: port=${process.env.ZHANGYUGE_AGENT_VITE_PORT}, config=${process.env.ZHANGYUGE_AGENT_CONFIG_DIR}`);
  }
}
...
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.ZHANGYUGE_AGENT_VITE_PORT || "5173";
  ...
  return {
    ...
    ZHANGYUGE_AGENT_CONFIG_DIR: process.env.ZHANGYUGE_AGENT_CONFIG_DIR || "",
    ZHANGYUGE_AGENT_APP_NAME: process.env.ZHANGYUGE_AGENT_APP_NAME || "章鱼哥AI",
    ZHANGYUGE_AGENT_DEEPLINK_SCHEME: process.env.ZHANGYUGE_AGENT_DEEPLINK_SCHEME || "zhangyuge-agent",
    ZHANGYUGE_AGENT_INSTANCE_NUMBER: process.env.ZHANGYUGE_AGENT_INSTANCE_NUMBER || "",
  };
}

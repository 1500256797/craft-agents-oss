...
VERSIONS_URL="https://agents.zhangyuge-agent.local/electron"
DOWNLOAD_DIR="$HOME/.zhangyuge-agent/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.lukilabs.zhangyuge-agent"
    if pgrep -x "章鱼哥AI" >/dev/null 2>&1; then
        info "Quitting 章鱼哥AI..."
        ...
        if pgrep -x "章鱼哥AI" >/dev/null 2>&1; then
            ...
            pkill -9 -x "章鱼哥AI" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  章鱼哥AI has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a '章鱼哥AI'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.zhangyuge-agent/app"
    WRAPPER_PATH="$INSTALL_DIR/zhangyuge-agent"
    APPIMAGE_INSTALL_PATH="$APP_DIR/章鱼哥AI-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "章鱼哥AI.*AppImage" >/dev/null 2>&1; then
        info "Stopping 章鱼哥AI..."
        pkill -f "章鱼哥AI.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# 章鱼哥AI launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.zhangyuge-agent/app/章鱼哥AI-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@zhangyuge-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@zhangyuge-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: 章鱼哥AI not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://agents.zhangyuge-agent.local/install-app.sh | bash"
    exit 1
fi
...
# AppImage creates a new /tmp/.mount_章鱼哥AI-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_章鱼哥AI' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/章鱼哥AI-x64.AppImage"
    ...
    echo ""
    printf "%b\n" "  Run with: ${BOLD}zhangyuge-agent${NC}"
    echo ""
